...
CREATE TABLE IF NOT EXISTS tip_uch (
  ...
  CONSTRAINT tip_uch_kod_uk UNIQUE (kod),
...
CREATE TABLE IF NOT EXISTS kom_oper (
  ...
  updated_at timestamp,                      -- Время обновления
  CONSTRAINT kom_oper_code_uk UNIQUE (opr_code),
...
CREATE TABLE IF NOT EXISTS uch_uzel (
  ...
  CONSTRAINT uch_uzel_uniq UNIQUE (uch, stan1, stan2, stan3)
);
...
CREATE TABLE IF NOT EXISTS tarif_stan (
  ...
  CONSTRAINT tarif_stan_uniq UNIQUE (uch, stan)
);
...
CREATE TABLE IF NOT EXISTS stan_zd (
  ...
  CONSTRAINT stan_zd_stan_uk UNIQUE (stan)
);
...
CREATE UNIQUE INDEX tarif_tr_pair_uk ON tarif_tr(tr_start, tr_end);
